...
def hardy_main(  # Data and Config Paths
    ...
    plot_format : str
                  option for standard or RGB color gradient. If 'RGBrgb' it
                  generates images in RGB format otherwise, it generates
                  cartesian plots
    print_out : bool
    ...
    classifier : str
                  option cnn or tuner
    scale :  float
             percentage for the image to reduce its size to.
    split : float
    ...
    skiprows: int
              Integer value representing the header size in the .csv files
...
def data_wrapper(run_name=None, raw_datapath='./', tform_command_dict=None,
    ...
    ----------
    run name: str
              String value representing the transfomation name if called by
              hardy_main() other wise can be any string
    raw_datapath: str
                   path to the raw .csv files containing the data to classify
    tform_command_dict: dict
                        dictionary containing the tranformation commands
                        extracted from the transfomation configuration file.
    ...
    iterator_mode : str
                    option to use images from arrays directly or save the
                    .png and use a directory iterator mode. If 'arrays',
                    it generates the arrays. Other values generates images
    print_out : bool
                option for printing out feedback on conputational time taken to
                initialize the data and generate the images
    project_name : str
                   name of the folder to be created for storing the results of
                   the tuning or for generated array/images data
    skiprows: int
              Integer value representing the header size in the .csv files
    scale :  float
             percentage for the image to reduce its size to.
    ...
    -------
    array_data: .pkl
                pickle file representing the images in the array format.
                Stored in raw data path
    images: .png
            png images in case mode iterator mode is not 'arrays'

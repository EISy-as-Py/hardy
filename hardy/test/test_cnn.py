...
from hardy.recognition import save_load_model
...
class TestSimulationTools(unittest.TestCase):
    ...
    def test_feature_maps(self):
        model_path = os.path.join('.', 'doc', 'examples')
        model = save_load_model(os.path.join(model_path, 'best_model.h5'),
                                load=True)
        image_name = 'exampleimage.png'
        image_path = os.path.join('.', 'hardy', 'test', 'test_image')
        last_layer = cnn.feature_map(image_name, model, 4, 100,
                                     layer_num='last',
        ...
        cnn.feature_map(image_name, model, 4, 100, layer_num=None,
                        image_path=image_path)
        cnn.feature_map(image_name, model, 4, 100, layer_num=2,
                        image_path=image_path)

        # test for array data
        image_load = load_img(os.path.join(image_path, image_name),
                              target_size=(100, 100))

        image_array = img_to_array(image_load)

        last_layer_array = cnn.feature_map(
            image_array, model, 4, 100, layer_num='last')
        ...
        cnn.feature_map(image_array, model, 4, 100, layer_num=None)

        cnn.feature_map(image_array, model, 4, 100, layer_num=2)

        # remove the images and folder just generated by the feature_map fnct.
        new_folder_path = os.path.join('.', 'report')
    ...
    def test_save_load_model(self):
        ...
        model, history = cnn.build_model(train, val,
                                         config_path=configuration)
        model_location = os.path.join('.', 'hardy', 'test', 'model')
        saved_model = cnn.save_load_model(
            model=model, save=True, filepath=model_location)
        assert saved_model == 'the model was correctly saved'
        model_loaded = cnn.save_load_model(
            load=True, filepath=model_location)
